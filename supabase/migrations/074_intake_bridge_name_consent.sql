-- Dubbele onboarding-stap wegnemen voor intake-respondenten.
--
-- Een respondent vult het publieke intakeformulier in (naam + AVG/voorwaarden-
-- consent) en wordt daarna uitgenodigd. Bij aanmelden koppelde migratie 073 al
-- de catalogus-antwoorden en `full_name` naar het profiel — maar NIET de
-- achternaam (het publieke formulier heeft één gecombineerd 'Naam'-veld) en NIET
-- de consent. ProfileCompletionGuard eist juist expliciet `last_name` én
-- `terms_accepted_at`/`terms_version`, dus verscheen de profiel-modal opnieuw met
-- een tweede keer naam + consent invullen.
--
-- Deze migratie breidt de intake-brug uit (fill-empty-only, niets overschrijven):
--   1. Splits `intake.name` naar voornaam/achternaam als fallback wanneer er geen
--      catalogus-vraag voor first_name/last_name bestaat. Achternaam alleen als de
--      naam een spatie bevat (anders blijft 'ie leeg i.p.v. de voornaam te dupliceren).
--   2. Neem de consent (`terms_accepted_at`, `terms_version`) over uit de intake.
--      Blijft de versie achter op CONSENT_VERSION, dan vraagt de guard alsnog om
--      hernieuwde toestemming — dat is gewenst gedrag.
--
-- Behoudt alle defensiviteit en search_path van 066/073.
    ...
    LOOP
      ...
      BEGIN
        ...
        UPDATE profiles p SET
          -- Naam: catalogus-antwoord > gesplitste intake-naam (fallback)
          ...
          -- Consent uit de intake overnemen zodat de profiel-guard 'm niet opnieuw vraagt
