-- Uitgenodigde intake-respondent = goedgekeurd, geen 'guest' meer.
--
-- `guest` betekent in de app "heeft zich aangemeld, wacht op beoordeling"
-- (Dashboard toont dan "Je aanvraag wordt beoordeeld"). Maar wie via het
-- intakeformulier door een beheerder wordt UITGENODIGD, is juist al beoordeeld
-- en goedgekeurd. Die hoort als `aspirant` binnen te komen (goedgekeurde
-- nieuwkomer → "Welkom als aspirant-lid!").
--
-- Wijziging t.o.v. 074: in de intake-brug wordt het membership nu met rol
-- 'aspirant' aangemaakt i.p.v. 'guest'. Verder identiek. ON CONFLICT DO NOTHING
-- blijft: een bestaand (hoger) lidmaatschap wordt nooit verlaagd.
--
-- Daarnaast een eenmalige backfill: bestaande `guest`-leden die via een
-- uitgenodigde-en-gejoinde intake binnenkwamen, alsnog naar `aspirant`. Scope
-- is strikt (alleen rijen met een matchende intake_responses op status 'joined'),
-- zodat zelf-aangemelde gasten die écht op beoordeling wachten ongemoeid blijven.
...
BEGIN
  ...
  BEGIN
    ...
    LOOP
      -- Uitgenodigd = goedgekeurd → aspirant (niet guest/'wacht op beoordeling')
      INSERT INTO memberships (profile_id, project_id, role)
      VALUES (new.id, intake.project_id, 'aspirant')
      ON CONFLICT (profile_id, project_id) DO NOTHING;
...
-- Eenmalige backfill: vastgelopen guests die via een gejoinde intake binnenkwamen.
UPDATE memberships m
SET role = 'aspirant'
FROM intake_responses ir
WHERE ir.project_id = m.project_id
  AND ir.profile_id = m.profile_id
  AND ir.status = 'joined'
  AND m.role = 'guest';
